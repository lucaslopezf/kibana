...
interface SourceRow {
  ...
  /**
   * Link to the data stream rendered by the Streams feature. When provided,
   * it replaces the plain-text rendering of `indexName` so the data stream
   * sits in the same row as the rest of the metadata (matching typography
   * and placement).
   */
...
export interface OverviewTabMetadataProps {
  ...
  /**
   * When set, prepends a `<Label>: <value>` row above the standard metadata.
   * The label is derived from `kind` (Index or Data stream) and the value is
   * either `streamLink` (the link rendered by the Streams feature) or the
   * plain `indexName`.
   */
  sourceRow?: SourceRow;
}
