  ...
  describe('source row', () => {
    it('renders the Index row when sourceRow is an index', () => {
      const { getByTestId, getByText } = render(
        <OverviewTabMetadata
          ...
          sourceRow={{ indexName: 'my-source', kind: METRIC_SOURCE_KIND.INDEX }}
        />
      );
    ...
    it('renders the Data stream row when sourceRow is a data stream', () => {
      const { getByTestId, getByText } = render(
        <OverviewTabMetadata
          metricItem={createMockMetric()}
          sourceRow={{ indexName: 'logs-foo-default', kind: METRIC_SOURCE_KIND.DATA_STREAM }}
        />
      );
    ...
    it('renders streamLink inside the source row when provided', () => {
      ...
      // Plain-text fallback is not rendered alongside the override.
