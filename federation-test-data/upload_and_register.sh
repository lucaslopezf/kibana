#!/usr/bin/env bash
#
# Upload NDJSON to S3 and register the S3 datasets via the `_query` API.
# Does NOT run on its own: run it manually when you want to upload to S3.
#
#   ./upload_and_register.sh
#
# Requirements:
#   - valid aws login (STS credentials in the default profile)
#   - local ES up at http://localhost:9200 (elastic:changeme)
#   - NDJSON already generated in ./ndjson (node timeshift.js)
#
set -euo pipefail
...
DATASETS=("weblogs" "flights" "ecommerce")
...
for name in "${DATASETS[@]}"; do
  ...
  curl -s -u "$ES_USER:$ES_PASS" -X POST "$ES/_query" \
    -H 'content-type: application/json' \
    -d "{\"query\":\"FROM ${name} | LIMIT 2\"}" | head -c 800
  echo
done
