#!/usr/bin/env bash
#
# Upload traces NDJSON to S3 and register the S3 datasets via the `_query` API.
#
#   ./upload_and_register_traces.sh
#
# Requirements:
#   - valid aws login (STS credentials in the default profile)
#   - local ES up at http://localhost:9200 (elastic:changeme)
#   - NDJSON already generated in ./ndjson (node gen_traces.js)
#
set -euo pipefail
...
DATASETS=("traces-apm" "traces-otel")
